...
where
    ...
    reusable_with_clone(FuncParser::new(move |pc| {
      ...
      match first_result {
        ParseResult::Success { parse_context, value, length } => {
          ...
          match next_result {
            ParseResult::Success { parse_context, value, length: next_length } => {
  ...
  where
    ...
    reusable_with_clone(FuncParser::new(move |pc| {
      ...
      match op_result {
        ParseResult::Success { parse_context, value: f, length: op_length } => {
          let next_parser = reusable_with_clone(self_clone.clone());
          let expr_result = next_parser.run(parse_context);
          
          match expr_result {
            ParseResult::Success { parse_context, value: y, length: expr_length } => {
              // 次の式の結果とデフォルト値を関数に適用
